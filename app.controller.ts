...
import { AppService, castVoteDto, delegateVotes, mintTokens } from './app.service';
...
export class AppController {
  ...
  @Post('request-tokens')
  async requestTokens(@Body() body: mintTokens): Promise<string> {
    return await this.appService.requestTokens(body.mintToAddress, body.tokenAmnt);
  }

  @Post('self-delegate')
  async selfDelegate(@Body() body: delegateVotes): Promise<string> {
